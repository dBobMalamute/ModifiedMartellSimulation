...
void LogNormalDistributionIdea::compute(double mean, double stdev)
{
    ...
    for(int i = -150; i< 151; i++)
    {
        ...
        preYValues.push_back(y_val);
    }

    //Normalize because sometimes I need to do that.
    double area = 0.0;
    for(size_t i = 1; i < preYValues.size(); i++)
    {
        area += (preYValues[i] + preYValues[i - 1]) * (xValues[i] - xValues[i-1]);
    }
    area /= 2.0;

    for(size_t i = 0; i < preYValues.size(); i++)
    {
        yValues.push_back(preYValues.at(i) / area);
    }
