...
void DistributionsPlugin::registerIdeas(std::shared_ptr<IdeaRegistry> ideaRegistry)
{
    ...
    ideaRegistry->registerIdea<AreaUnderDistributionIdea>();
    ideaRegistry->registerIdea<ValueAtDistributionPointIdea>();

    qmlRegisterType(QUrl("qrc:/QML/DistributionDisplay.qml"),
                    "com.distributions.dists", 1, 0, "DistributionDisplay");
...
void DistributionsPlugin::registerToolbars(std::shared_ptr<ToolbarRegistry> toolbarRegistry)
{
    ...
    distributionUtilitiesList << Magic<ValueAtDistributionPointAttributes>::cast().listName();
    distributionUtilitiesList << Magic<AreaUnderDistributionAttributes>::cast().listName();
